...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `catalogo_02`.`stac_item` (`id` INT, `collection` INT, `date` INT, `path` INT, `row` INT, `cloud_cover` INT, `thumbnail` INT, `satellite` INT, `sensor` INT, `processing_level` INT, `radiometric_processing` INT, `geometric_processing` INT, `assets` INT, `TL_Longitude` INT, `TL_Latitude` INT, `BL_Longitude` INT, `BL_Latitude` INT, `BR_Longitude` INT, `BR_Latitude` INT, `TR_Longitude` INT, `TR_Latitude` INT);

-- -----------------------------------------------------
-- Placeholder table for view `catalogo_02`.`product`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `catalogo_02`.`product` (`id` INT, `resolution` INT, `processing_date` INT, `filename` INT, `band` INT, `geometric_processing` INT, `satellite` INT, `sensor` INT, `processing_level` INT, `radiometric_processing` INT, `scene_id` INT, `dataset_id` INT);
...
CREATE OR REPLACE VIEW `dataset` AS
    SELECT 
        d.id id,
        CONCAT(se.name, '_', sr.name, '_', p.name, '_', r.name) name,
...
CREATE OR REPLACE VIEW `stac_item` AS
    SELECT 
        ...
        s.path,
        s.row,
        s.cloud_cover_q1 cloud_cover,
        s.thumbnail thumbnail,
        d.satellite,
        d.sensor,
        d.processing_level,
        d.radiometric_processing,
        p.geometric_processing,
        CONCAT('[',
                GROUP_CONCAT(
